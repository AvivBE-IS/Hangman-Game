...
# Working with the with statement is recommended because it frees the system resources (such as files,
# synchronization objects, network connections, etc.), as we learned about closing a file. The with statement opens a
# block of code when it is finished the file will be closed
# When working with large files, it is recommended to use another method: go over one line at a time, using a for loop.


def main():
    welcome()
    secret_word = get_secret_word()
    # secret word is a tuple.
    num_of_tries = 1
    old_letters_guessed = []
    # The variable can be set as global. But the assignment has an automatic testing system and this disrupts it.

    while True:
        letter_guessed = input(f"Guess a letter: ")
        if try_update_letter_guessed(letter_guessed, old_letters_guessed):
            print(show_hidden_word(secret_word, old_letters_guessed))
        if letter_guessed not in list(secret_word[1]):
            num_of_tries += 1
            print("old letter guessed: ", show_old_letters_guessed(old_letters_guessed))
            print_hangman(num_of_tries)
        # The function is not necessary. It is more efficient to calculate MAX_TRIES-num_of_tries+1.
            print(show_how_tries_left(num_of_tries))
